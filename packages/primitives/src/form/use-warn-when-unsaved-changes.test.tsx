...
describe("useWarnWhenUnsavedChanges", () => {
	...
	test.each([
		["simple", "First Name"],
		["nested", "Author"],
	])("should not warn when navigating to a sub page of a form with submit button after updating %s field", async (_, field) => {
		window.confirm = vi.fn().mockReturnValue(true);
		render(<App />);
		fireEvent.change(screen.getByLabelText(field), {
			target: { value: "John Doe" },
	...
	test.each([
		["simple", "First Name"],
		["nested", "Author"],
	])("should not warn when navigating from a sub page of a form to the root part with submit button after updating %s field", async (_, field) => {
		window.confirm = vi.fn().mockReturnValue(true);
		render(<App initialEntries={["/form/part1"]} />);
		fireEvent.change(screen.getByLabelText(field), {
			target: { value: "John Doe" },
