...
import { englishMessages } from "@runes/languages";
...
describe("Form", () => {
	...
	it.each([
	...
	])("should support prefilling the from values from the location $from", async ({
		url,
		expectedValue,
	}) => {
		...
		expect((screen.getByText("Submit") as HTMLInputElement).disabled).toEqual(
			false,
		);
		fireEvent.click(screen.getByText("Settings"));
		await screen.findByDisplayValue(expectedValue);
		expect(screen.getByText<HTMLInputElement>("Submit").disabled).toEqual(
			false,
		);
	});
	it.each([
	...
	])("should support overriding the record values from the location $from", async ({
		url,
		defaultValues,
		expectedValue,
		expectedDefaultValue,
		...
		expect(
			(await screen.findByLabelText<HTMLInputElement>("category")).value,
