...
import { type ChoicesContextValue, Form } from "../../form";
import type { GetManyResult, RaRecord } from "../../types";
import { SORT_ASC } from "../list/query-reducer";
import {
	type UseReferenceArrayInputParams,
	useReferenceArrayInputController,
} from "./use-reference-array-input-controller";
...
describe("useReferenceArrayInputController", () => {
	...
	describe("isLoading", () => {
		it("should set isLoading to true as long as there are no references fetched and no selected references", () => {
			...
			render(
				...
				>
					<Form defaultValues={{ tag_ids: [1, 2] }} onSubmit={vi.fn()}>
		...
		it("should set isLoading to false once the dataProvider returns", async () => {
			...
			render(
				<CoreAdminContext dataProvider={dataProvider}>
					<Form onSubmit={vi.fn()} defaultValues={{ tag_ids: [1, 2] }}>
						<ReferenceArrayInputController {...defaultProps}>
							{children}
						</ReferenceArrayInputController>
					</Form>
	...
	it("should call getMany on mount if value is set", async () => {
		...
		const dataProvider = testDataProvider({
			...
			getMany: jest
				.fn()
				.mockResolvedValue(Promise.resolve({ data: [{ id: 5 }, { id: 6 }] })),
		});
		render(
			<CoreAdminContext dataProvider={dataProvider}>
