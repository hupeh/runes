...
import type { UseRecordFromLocationOptions } from "../form";
import {
	getRecordFromLocation,
	useRecordFromLocation,
} from "./use-record-from-location";
...
describe("useRecordFromLocation", () => {
	...
	it("return the record from the location search", async () => {
		...
		render(
			<TestMemoryRouter
				initialEntries={[`/posts/create?source=${JSON.stringify(record)}`]}
			>
				<UseGetRecordFromLocation />
			</TestMemoryRouter>,
		);
	...
	it("return the record from the location state", async () => {
		...
		render(
			<TestMemoryRouter
				initialEntries={[{ pathname: `/posts/create`, state: { record } }]}
			>
				<UseGetRecordFromLocation />
			</TestMemoryRouter>,
		);
