...
import { type InputProps, type UseInputValue, useInput } from "./use-input";
import { DefaultValue } from "./use-input.stories";
import { required } from "./validation/validate";

const Input: FunctionComponent<
	{
		children: (props: ReturnType<typeof useInput>) => ReactNode;
	} & InputProps
> = (props) => {
...
describe("useInput", () => {
	it("returns the props needed for an input", () => {
		let inputProps;
		render(
			<CoreContext dataProvider={testDataProvider()}>
				<Form onSubmit={vi.fn()}>
					<Input
			...
			<CoreContext dataProvider={testDataProvider()}>
				<Form onSubmit={vi.fn()}>
					...
					>
						{({ id, field }) => {
							return (
								<input type="text" id={id} aria-label="Title" {...field} />
							);
						}}
					</Input>
				</Form>
	...
	describe("defaultValue", () => {
				...
				<CoreContext dataProvider={testDataProvider()}>
					<Form onSubmit={vi.fn()} record={{ id: 1, views: 0 }}>
						<Input source="views" resource="posts" defaultValue={99}>
							{({ id, field }) => {
								return (
									<input type="number" id={id} aria-label="Views" {...field} />
								);
							}}
						</Input>
					</Form>
		...
		it("does not apply the defaultValue true when the field is of type checkbox and has a value", () => {
			render(
				<CoreContext dataProvider={testDataProvider()}>
					<Form record={{ id: 1, is_published: false }} onSubmit={vi.fn()}>
						<BooleanInput source="is_published" defaultValue={true} />
					</Form>
				</CoreContext>,
			);
		...
		it("does not apply the defaultValue false when the field is of type checkbox and has a value", () => {
			render(
				<CoreContext dataProvider={testDataProvider()}>
					<Form record={{ id: 1, is_published: true }} onSubmit={vi.fn()}>
						<BooleanInput source="is_published" defaultValue={false} />
					</Form>
				</CoreContext>,
			);
