...
import type { SaveHandler } from "../controller";
import { FormGroupsProvider } from "./groups/form-groups-provider";
import { useAugmentedForm } from "./use-augmented-form";
import type { ValidateForm } from "./validation/get-simple-validation-resolver";
import { WarnWhenUnsavedChanges } from "./warn-when-unsaved-changes";
...
export function Form<RecordType = any>(props: FormProps<RecordType>) {
	...
	const getSource = (source: string) => source;
	const getLabel = (source: string) => `resources.${resource}.fields.${source}`;
	...
	return (
		<DataContextProvider value={record}>
			<SourceContextProvider getSource={getSource} getLabel={getLabel}>
				<FormProvider {...form}>
					<FormGroupsProvider>
						<form
