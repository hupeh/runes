import { CoreContext, type DataProvider } from "@runes/core";
...
const EditForm = () => {
	...
	return (
		<Form defaultValues={{ name: "John Doe" }}>
			<p>
				The name field should be unique. Try to enter "John Doe". Jane Doe
				should work as this is the current record value
			</p>
			<Input source="name" defaultValue="" validate={unique()} />
			<button type="submit">Submit</button>
		</Form>
...
const WithTranslatedMessageForm = () => {
	...
	return (
		<Form defaultValues={{ name: "John Doe" }}>
			...
			<p>
				Show that the value and the field label are passed to the translation
				provider to build the validation message
			</p>
...
const WithAdditionalFiltersForm = () => {
	...
	return (
		<Form defaultValues={{ name: "John Doe", organization_id: 1 }}>
			<p>
				The name field should be unique. "John Doe" is already registered for
				BigCorp and "Jane Doe" is already registered for EvilCorp.
			</p>
