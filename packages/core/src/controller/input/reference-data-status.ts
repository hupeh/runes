...
import type { RaRecord, TranslateFunction } from "../../types";
import type { MatchingReferencesError } from "./types";
...
export const getStatusForInput = <RecordType extends RaRecord = RaRecord>({
	...
	return {
		...
		error:
			(field.value && selectedReferenceError && matchingReferencesError) ||
...
export const getStatusForArrayInput = <RecordType extends RaRecord = any>({
	...
	return {
		...
		warning:
			matchingReferencesError ||
			(field.value && selectedReferencesDataStatus !== REFERENCES_STATUS_READY)
				? matchingReferencesError ||
